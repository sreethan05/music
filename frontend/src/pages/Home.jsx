        ...
        <div className="w-full">
          ...
          <div className="w-full h-[220px] rounded-[28px] overflow-hidden relative border border-slate-200/50 shadow-[0_15px_35px_rgba(0,0,0,0.015)] group">
            ...
            <div className="absolute inset-0 flex flex-col justify-center items-start px-7 md:px-10 z-10 select-none">
              ...
              <h2 className="text-2xl md:text-3xl font-black text-slate-800 tracking-tight leading-none mt-2 max-w-sm md:max-w-md truncate">
                {featuredSong.name}
              </h2>
              <p className="text-xs text-slate-400 font-extrabold mt-1 max-w-[280px] truncate">
              ...
              <div className="flex flex-wrap items-center gap-3 mt-6">
                ...
                <div className="border border-slate-300 bg-white/75 text-slate-700 font-extrabold text-[10.5px] tracking-wider uppercase px-5 py-2.5 rounded-full shadow-sm flex items-center gap-2">
              ...
              <div className="absolute bottom-6 right-8 text-right hidden md:block">
                <span className="text-[9px] font-extrabold tracking-wider text-slate-400 uppercase">Library Size</span>
                <p className="text-sm font-black text-slate-800 tracking-tight">{songsData.length.toLocaleString()} songs</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {!isLibraryLoading && filteredSongs.length === 0 ? (
          ...
          <div className="w-full flex flex-col gap-1.5 pb-20">
                ...
                <div
                  ...
                  className={`grid grid-cols-12 px-4 md:px-6 py-3.5 items-center text-xs transition-all duration-200 cursor-pointer group ${
                    isPlaying
                      ...
                      : "text-slate-500 hover:bg-white/45 hover:text-slate-800 hover:scale-[1.002]"
                  }`}
                >
                  ...
                  <div className="col-span-2 text-center font-bold tabular-nums flex items-center justify-center gap-2">
                    ...
                    <button
                      ...
                      className="opacity-100 md:opacity-0 group-hover:opacity-100 w-6 h-6 rounded-full bg-slate-100 hover:bg-slate-200 flex items-center justify-center text-slate-650 hover:text-slate-900 transition-all hover:scale-110 active:scale-95 cursor-pointer shrink-0"
                      title="Add to Playlist"
                    >
                      <Plus className="w-3.5 h-3.5 stroke-[2.5]" />
                    </button>
                  </div>
                  <span className="col-span-4 md:col-span-2 truncate font-bold text-slate-400">{song.album || "Single"}</span>
                </div>
              );
            })}
            ...
            {hasMoreSongs && (
              <button
                onClick={() => setVisibleCount((count) => Math.min(count + 60, filteredSongs.length))}
                className="mt-4 w-full py-3.5 bg-white border border-slate-200/70 hover:bg-slate-50 text-slate-700 rounded-2xl text-xs font-black uppercase tracking-widest transition-all active:scale-[0.99] cursor-pointer"
              >
                Load 60 More Tracks
              </button>
            )}
